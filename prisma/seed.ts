...
async function main() {
  const passwordHash = process.env.ADMIN_PASSWORD_HASH ?? await bcrypt.hash("Admin1234!", 12);
  ...
  await prisma.service.createMany({
    ...
    data: [
      ...
      { name: "Consultanță digitală", description: "Clarificăm produsul, arhitectura și pașii tehnici înainte de investiții mari.", icon: "Lightbulb", features: ["Discovery tehnic", "Roadmap de produs", "Estimări realiste"], order: 6 },
  ...
  await prisma.servicePackage.createMany({
    data: [
      { name: "Start tehnic", description: "Pentru companii care vor validare rapidă și un plan clar de execuție.", price: "de la 3.500 EUR", duration: "2-4 săptămâni", deliverables: ["Audit tehnic", "Plan de arhitectură", "Estimare de buget"], targetIndustries: ["SaaS", "E-commerce", "Servicii profesionale"], cta: "Programează analiza" },
      { name: "Echipă dedicată", description: "Pentru dezvoltare continuă cu specialiști integrați în procesele tale.", price: "ofertă lunară personalizată", duration: "minimum 3 luni", deliverables: ["Ingineri dedicați", "Lead tehnic", "Raportare săptămânală"], targetIndustries: ["Fintech", "Healthcare", "Manufacturing"], cta: "Cere o echipă" },
  ...
  const projects = [
    {
      ...
      description: "Portal self-service pentru cereri de ofertă, fluxuri de aprobare și raportare comercială.",
    ...
    {
      title: "Dashboard financiar pentru analiză de portofoliu",
  ...
  await prisma.contactLead.createMany({
    data: [
      ...
      { name: "Mihai Rusu", company: "Medica Plus", email: "mihai.rusu@medicaplus.ro", phone: "+40 722 000 200", serviceNeeded: "Echipe dedicate", message: "Căutăm doi dezvoltatori full-stack pentru extinderea produsului nostru operațional.", budgetRange: "peste 50.000 EUR", timeline: "sub 1 lună" },
    ],
  });
}
